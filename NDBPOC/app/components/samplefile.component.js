...
var samplefile = (function () {
    ...
    samplefile.prototype.onSubmit = function (formData) {
        ...
        switch (this.dbops) {
            case enum_1.DBOperation.create:
                ...
                var insertStatus_1 = false;
                if (myFile) {
                    var formDataLocal = new FormData(); // this is using to create local file
                    formDataLocal.append('uploadFile', myFile, myFile.name);
                    this._sampleFileService.fileupload(global_1.Global.BASE_FILESAVE_ENDPOINT, formDataLocal)
                        .subscribe(function (myMsg) {
                        _this.msg = myMsg._body;
                        if (_this.msg == "409") {
                            _this.msg = "The File Is Exists";
                            insertStatus_1 = false;
                            _this.modal.dismiss();
                        }
                        else {
                            insertStatus_1 = true;
                        ...
                        if (insertStatus_1) {
                            formData._value.FilePath = global_1.Global.BASE_FOLDER_PATH + _this.FileDetails.name;
                            formData._value.ApprovalStatus = "Initiated";
                            _this._sampleFileService.post(global_1.Global.BASE_SAMPLEFILE_ENDPOINT, formData._value).subscribe(function (data) {
                                if (data == 1) {
    ...
    samplefile.prototype.GetHTMLTEMPLATEDataFromDB = function () {
        var _this = this;
    ...
    samplefile.prototype.viewPDF = function (data) {
        var _this = this;
        this.indLoading = true;
        ...
        }, function (error) { return _this.msg = error; }, function () { return _this.GeneratePDFInDetail(); });
    };
    samplefile.prototype.GetFileContentFirstRow = function (id) {
        var _this = this;
        this.file = this.files.filter(function (x) { return x.FID == id; })[0];
        var val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        var fileNameOnly = val.split('.')[0];
    ...
    samplefile.prototype.GetFileFullDataToDisplayViaGrid = function (id) {
        var _this = this;
        this.file = this.files.filter(function (x) { return x.FID == id; })[0];
        var val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        var fileNameOnly = val.split('.')[0];
        ...
        this._sampleFileService.getLoginInfo(global_1.Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            .subscribe(function (localFileDataX) {
            localFileData = localFileDataX;
            _this.CSVfileContent = localFileData.toString().replace(/(?:\r\n|\r|\n)/g, '<br /><br/>');
        }, function (error) { return _this.msg = error; }, function () { return _this.ViewSampleFile(id); });
    };
    samplefile.prototype.GetFileFullDataToDisplayViaPopUp = function (fileData) {
        var _this = this;
        var id = fileData._value.FID;
        this.file = this.files.filter(function (x) { return x.FID == id; })[0];
        var val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        var fileNameOnly = val.split('.')[0];
        var localFileData = '';
    ...
    samplefile.prototype.GeneratePDFInDetail = function () {
        var doc = new jsPDF();
        ...
        doc.text(30, 40, this.firstRowdata.split(',')[0] + ",");
        ////HTML Data
        ...
        //doc.setFontSize(12)
        //doc.text(20, 45, this.fileContent.split(',')[7]);
        //main content begin
        doc.setFontType('normal');
        doc.setFontSize(12);
        doc.text(30, 55, this.firstRowdata.split(',')[7]);
        doc.setFontType('normal');
        doc.setFontSize(12);
        doc.text(50, 70, 'Requested Shares:');
        doc.setFontType('bold');
        doc.setFontSize(12);
        doc.text(90, 70, this.firstRowdata.split(',')[1]);
        doc.setFontType('normal');
        doc.setFontSize(12);
        doc.text(50, 75, 'Issued Shares:');
        doc.setFontType('bold');
        doc.setFontSize(12);
        doc.text(90, 75, this.firstRowdata.split(',')[2]);
        doc.setFontType('normal');
        doc.setFontSize(12);
        doc.text(50, 80, 'Rejected Shares:');
        doc.setFontType('bold');
        doc.setFontSize(12);
        doc.text(90, 80, this.firstRowdata.split(',')[3]);
        //main content end
        doc.setFontSize(12);
        doc.setFontType('normal');
        doc.text(30, 95, 'Please find below, a cheque for the value of rejected shares.');
        doc.setFontSize(9);
        doc.setFontType('normal');
        doc.setTextColor(150);
        doc.text(50, 140, '*This is a computer generated document and so requires no signature.*');
        //main cheque
    ...
    samplefile.prototype.HideGenerateProofButtonByStatus = function (status) {
    ...
    samplefile.prototype.HideFinalPrintButtonByStatus = function (status) {
    ...
    samplefile.prototype.HideEditDeleteButtonByStatus = function (status) {
        if (status == "Initiated") {
            return false;
        }
        else {
    ...
    samplefile.prototype.HideFirstPrintOK = function (status) {
    };
    return samplefile;
}());
__decorate([
...
__decorate([
    ...
    __metadata("design:type", ng2_bs3_modal_1.ModalComponent)
], samplefile.prototype, "viewCsv", void 0);
__decorate([
    core_1.ViewChild('mymodalID'),
    __metadata("design:type", ng2_bs3_modal_1.ModalComponent)

...
export class samplefile implements OnInit {
    ...
    isEvidenceFileUploadVisible = false;
    ...
    generateProofPOPUP(id: number) {
        ...
        if (this.file.EvidenceFilePath != null) {
            this.isEvidenceFileUploadVisible = false;
        }
        else {
            this.isEvidenceFileUploadVisible = true;
        }
    ...
    ViewEvidenceFile() {
        window.open('https://drive.google.com/file/d/0B1guDx79ldfIQWJtOFdlVFB3dFk/view?usp=sharing');
    }
    ...
    GetActualFileFullData(fileFrm: any) {
        ...
        let val = this.file.FilePath.split('/')[this.file.ActualFilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';

...
export class samplefile implements OnInit {
    ...
    @ViewChild('myActualFileApproval') myActualFileApprovalID: ModalComponent;
    @ViewChild('myActualFile') myActualFilePopup: ModalComponent;
    ...
    isEvidenceFileUploaded = false;
    isActualFileUploaded = false;
    ...
    getFileDetailsEvidence(event: any): void {
        this.EvidenceFileDetails = event.target.files[0];
    }

    getFileDetailsActualFile(event: any): void {
        this.ActualFileDetails = event.target.files[0];
    }
    ...
    fileUpload() {
        ...
        //        .subscribe(
        //        myMsg => {
        //            this.msg = myMsg._body;
        //            if (this.msg == "409") {
        //                this.msg = "The File Is Exists"
        //                insertStatus = false;
        //            }
        //            else {
        //                insertStatus = true;
    ...
    onSubmit(formData: any) {
        ...
        switch (this.dbops) {
            case DBOperation.create:
                //let saveStatus = ""//this.fileUpload();
                let myFile: File = this.FileDetails;
                let insertStatus = false;
                if (myFile) {
                    let formDataLocal = new FormData(); // this is using to create local file
                    formDataLocal.append('uploadFile', myFile, myFile.name);
                    this._sampleFileService.fileupload(Global.BASE_FILESAVE_ENDPOINT, formDataLocal)
                        .subscribe(
                        myMsg => {
                            this.msg = myMsg._body;
                            if (this.msg == "409") {
                        ...
                        error => this.msg = <any>error,
                        () => {
                            if (insertStatus) { // this is to create data in DATABASE
                                formData._value.FilePath = Global.BASE_FOLDER_PATH + this.FileDetails.name;
    ...
    generateProofPOPUP(id: number) {
        ...
        //Must go through the logic even DATA from local FILE or from DATABASE COLUMN
        if (this.file.ApprovalStatus == "Proof Created" || this.file.ApprovalStatus == "Sent For Approval" || this.file.ApprovalStatus == "Evidence Attached") {
        ...
        this.GetFileContentFirstRow(id);

        //This is EDIT, already SAVED as PROOF CREATED
        if (this.file.Description != null && this.file.Description.length > 0) {           
            this.htmlTemplateData = this.file.Description;
            this.templateVal = this.htmlTemplateData;
        }
        //THIS is for FIRST TIME
        else {
            this.GetHTMLTEMPLATEDataFromDB();
        }

        //this is for new requirement
        if (this.file.EvidenceFilePath != null) {
            this.isEvidenceFileUploaded = true;
        }
        else {
            this.isEvidenceFileUploaded = false;
        }
        //alert(this.isEvidenceFileUploaded);
    ...
    ViewActualPOPUP(id: number) {
        this.file = this.files.filter(x => x.FID == id)[0];
        this.fileFrm.setValue(this.file);
        this.SetControlsState(true);
        this.modalTitle = "Approve Actual File";
        this.myActualFileApprovalID.open();
    }

    ActualFilePopup(id: number) {
        this.file = this.files.filter(x => x.FID == id)[0];
        //this is for new requirement
        if (this.file.ActualFilePath != null) {
            this.isActualFileUploaded = true;
        }
        else {
            this.isActualFileUploaded = false;
        }

        this.fileFrm.setValue(this.file);
        this.SetControlsState(true);
        this.modalTitle = "Actual File";
        this.myActualFilePopup.open();
    }
    ...
    ApproveActualFile(paraFrm: any) {
        paraFrm._value.ApprovalStatus = "Actual File Approved";
        paraFrm._value.ProofAuthor = "ChanakaG";
        //paraFrm._value.ProofTime = Date.now();
        ...
        this._sampleFileService.put(Global.BASE_SAMPLEFILE_ENDPOINT, paraFrm._value.FID, paraFrm._value).subscribe(
            data => {
                ...
                this.myActualFileApprovalID.dismiss();
            },
            error => {
                this.msg = error;
            }
    ...
    RejectActualFile(paraFrm: any) {
        paraFrm._value.ApprovalStatus = "Actual File Rejected";
        paraFrm._value.ProofAuthor = "ChanakaG";
        //paraFrm._value.ProofTime = Date.now();
    ...
    ViewEvidenceFile() {
        window.open('https://drive.google.com/file/d/0B1guDx79ldfIcGFWNE1kVWZRUlU/view');
    }
    ...
    GenerateProofWithEvidence(fileFrm: any) {
        fileFrm._value.ApprovalStatus = "Evidence Attached";

        let myFile: File = this.EvidenceFileDetails;
        let insertStatus = false;
        if (myFile) {
            let formDataLocal = new FormData(); // this is using to create local file
            formDataLocal.append('uploadFile', myFile, myFile.name);
            this._sampleFileService.fileupload(Global.BASE_FILESAVE_ENDPOINT, formDataLocal)
                ...
                () => {
                    if (insertStatus) { // this is to create data in DATABASE
                        fileFrm._value.EvidenceFilePath = Global.BASE_FOLDER_PATH + this.EvidenceFileDetails.name;
                        fileFrm._value.FILEADD = null;

                        this._sampleFileService.put(Global.BASE_SAMPLEFILE_ENDPOINT, fileFrm._value.FID, fileFrm._value).subscribe(
                            data => {
                                if (data == 1) //Success
                                {
                                    this.msg = "Data successfully updated.";
    ...
    viewPDF(data: any): void {
        ...
        this._sampleFileService.getLoginInfo(Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            ...
            error => this.msg = <any>error,
            () => this.GeneratePDFInDetail(data)
            );
    }

    GetFileContentFirstRow(id: number) {
        this.file = this.files.filter(x => x.FID == id)[0];
        let val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';
        ...
        this._sampleFileService.getLoginInfo(Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            .subscribe(localFileDataX => {
                localFileData = localFileDataX;
                this.firstRowdata = localFileData.split("\r\n")[1];
                //alert(this.firstRowdata)
            },
            error => this.msg = <any>error);

    }

    GetFileFullDataToDisplayViaGrid(id: number) {
        this.file = this.files.filter(x => x.FID == id)[0];
        let val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';
        ...
        this._sampleFileService.getLoginInfo(Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            .subscribe(localFileDataX => {
                localFileData = localFileDataX;
                this.CSVfileContent = localFileData.toString().replace(/(?:\r\n|\r|\n)/g, '<br /><br/>');
            },
            error => this.msg = <any>error,
            () => this.ViewSampleFile(id)
            );

    }

    GetFileFullDataToDisplayViaPopUp(fileData: any) {
        let id = fileData._value.FID;
        this.file = this.files.filter(x => x.FID == id)[0];
        let val = this.file.FilePath.split('/')[this.file.FilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';
    ...
    FinalPrintPOPUP(id: number) {
        ...
        //alert(this.enablePrintAllButton)

        //PRINT FIRST RECORD
        if (this.file.isPrintFirstRecord) {
            this.enablePrintFirstRecordButton = false;
        }
        else {
            this.enablePrintFirstRecordButton = true;
        }

        //if (this.file.ApprovalStatus == "First Print OK" || this.file.ApprovalStatus == "Print All Done") {
    ...
    HideApproveActualFileButtonByStatus(status: string) {
        if (status == "Actual File Uploaded") {
            return false;
        }
        else {
            return true;
    ...
    HideGenerateProofButtonByStatus(status: string) {
        ...
        if (status == "Sample File Approved" || status == "Proof Created" || status == "Evidence Attached") {
            return false;
        }
        else {
            return true;
    ...
    HideFinalPrintButtonByStatus(status: string) {
        if (status == "Actual File Approved" || status == "First Print OK" || status == "Print All Done") {
            return false;
        }
        else {
            return true;
    ...
    HideActualFileUploadButtonByStatus(status: string) {
        if (status == "Proof Approved") {
            return false;
        }
        else {
            return true;
    ...
    PrintAllRecords() {
        let val = this.file.ActualFilePath.split('/')[this.file.ActualFilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';
        ...
        this._sampleFileService.getLoginInfo(Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            ...
            error => this.msg = <any>error,
            () => this.PrintAllRecordsWithFile(this.CSVfileContent)
            );
    }

    UploadActualFile(fileFrm: any) {
        fileFrm._value.ApprovalStatus = "Actual File Uploaded";
        let myFile: File = this.ActualFileDetails;
            ...
            this._sampleFileService.fileupload(Global.BASE_FILESAVE_ENDPOINT, formDataLocal)
                ...
                myMsg => {
                    ...
                    if (this.msg == "409") {
                        this.msg = "The File Is Exists"
                        insertStatus = false;
                        this.myActualFilePopup.dismiss();
                    }
                    else {
                        insertStatus = true;
                    }
                ...
                () => {
                    if (insertStatus) { // this is to create data in DATABASE
                        fileFrm._value.ActualFilePath = Global.BASE_FOLDER_PATH + this.ActualFileDetails.name;
                        fileFrm._value.FILEADD = null;
    ...
    //Generating PDF for FIRST Record
    GeneratePDFInDetail(data: any) {
        var doc = new jsPDF();
        //logo
        ...
        //doc.setFontSize(12)
        //doc.setFontType('bold')
        //doc.text(30, 80, this.firstRowdata.split(',')[0] + ",")

        //main content begin
        //doc.setFontType('normal')
        //doc.setFontSize(12)
        //doc.text(30, 95, this.firstRowdata.split(',')[7]);

        //doc.setFontType('normal')
        //doc.setFontSize(12)
        //doc.text(50, 110, 'Requested Shares:')

        //doc.setFontType('bold')
        //doc.setFontSize(12)
        //doc.text(90, 110, this.firstRowdata.split(',')[1])

        //doc.setFontType('normal')
        //doc.setFontSize(12)
        //doc.text(50, 115, 'Issued Shares:')

        //doc.setFontType('bold')
        //doc.setFontSize(12)
        //doc.text(90, 115, this.firstRowdata.split(',')[2])

        //doc.setFontType('normal')
        //doc.setFontSize(12)
        //doc.text(50, 120, 'Rejected Shares:')

        //doc.setFontType('bold')
        //doc.setFontSize(12)
        //doc.text(90, 120, this.firstRowdata.split(',')[3])
        //main content end

        doc.setFontSize(12)
        doc.setFontType('normal')
        doc.text(53, 175, 'Please find below, a cheque for the value of rejected shares.');

        doc.setFontSize(9)
        doc.setFontType('normal')
        doc.setTextColor(150)
        doc.text(60, 190, '*This is a computer generated document and so requires no signature.*');


        //main cheque
        doc.rect(0, 200, 210, 100)
        //image
        doc.addImage(imgData, 'JPEG', 0, 202, 90, 30)
        ...
        doc.setTextColor(0)
        doc.setFontSize(14)
        doc.text(145, 224, 'Date');
        //yyyy
        doc.text(196, 224, yy.toString().charAt(3));
        doc.rect(160, 220, 5, 5)
        doc.text(191, 224, yy.toString().charAt(2));
        doc.rect(165, 220, 5, 5)
        doc.text(186, 224, yy.toString().charAt(1));
        doc.rect(170, 220, 5, 5)
        doc.text(181, 224, yy.toString().charAt(0));
        doc.rect(175, 220, 5, 5)
        //mm
        if (mm.toString().length < 2) {
            doc.text(176, 224, mm.toString().charAt(0));
            doc.rect(180, 220, 5, 5)
            doc.text(171, 224, '0');
            doc.rect(185, 220, 5, 5)
        }
        else {
            doc.text(176, 224, mm.toString().charAt(1));
            doc.rect(180, 220, 5, 5)
            doc.text(171, 224, mm.toString().charAt(0));
            doc.rect(185, 220, 5, 5)
        }
        //dd
        if (dd.toString().length < 2) {
            doc.text(166, 224, dd.toString().charAt(0));
            doc.rect(190, 220, 5, 5)
            doc.text(161, 224, '0');
            doc.rect(195, 220, 5, 5)
        }
        else {
            doc.text(166, 224, dd.toString().charAt(1));
            doc.rect(190, 220, 5, 5)
            doc.text(161, 224, dd.toString().charAt(0));
            doc.rect(195, 220, 5, 5)
        }
        ...
        //text line 1:1
        doc.setFontSize(14)
        doc.text(7, 240, 'Pay to ');
        //text line 1:2
        doc.setFontSize(20)
        doc.text(25, 240, '**' + this.firstRowdata.split(',')[0] + '**');

        //text line 2:1
        doc.setFontSize(14)
        doc.text(7, 250, 'The sum of Rupees');
        //text line 2:2
        doc.setFontSize(20)
        doc.text(7, 260, '**' + this.firstRowdata.split(',')[8] + '**');
        ...
        //text line 3:2
        doc.setFontSize(20)
        doc.text(150, 250, '**' + this.firstRowdata.split(',')[5] + '**');

        //text line 4:1
        doc.setFontSize(14)
        doc.text(150, 290, 'Authorized Officer');


        doc.save("NDB_CMS_Cheque_" + Date().toString() + ".pdf");
        doc.output('dataurlnewwindow');

    }

    //Generating PDF for ALL
    PrintAllRecordsWithFile(fileContent: any) {
        var doc = new jsPDF();

        if (fileContent) {
            let data = fileContent.split('<br/><br/>');
            var doc = new jsPDF();
            for (var i = 1; i < data.length - 1; i++) {
                ...
                doc.setFontSize(12)
                doc.setFontType('bold')
                doc.text(30, 80, data[i].split(',')[0] + ",")

                //main content begin
                doc.setFontType('normal')
                doc.setFontSize(12)
                doc.text(30, 95, data[i].split(',')[7]);
                ...
                doc.setFontType('bold')
                doc.setFontSize(12)
                doc.text(90, 110, data[i].split(',')[1])
                ...
                doc.setFontType('bold')
                doc.setFontSize(12)
                doc.text(90, 115, data[i].split(',')[2])
                ...
                doc.setFontType('bold')
                doc.setFontSize(12)
                doc.text(90, 120, data[i].split(',')[3])
                //main content end

                doc.setFontSize(12)
                doc.setFontType('normal')
                doc.text(30, 135, 'Please find below, a cheque for the value of rejected shares.');
                ...
                //text line 1:2
                doc.setFontSize(20)
                doc.text(25, 240, '**' + data[i].split(',')[0] + '**');
                ...
                //text line 3:2
                doc.setFontSize(20)
                doc.text(150, 250, '**' + data[i].split(',')[5] + '**');
            ...
            doc.save("NDB_CMS_Cheque_All" + Date().toString() + ".pdf");
            doc.output('dataurlnewwindow');
    ...
    viewApprovedPDF() {
        window.open('https://drive.google.com/file/d/0B1guDx79ldfIQzFpRlE0U3ZIRU0/view');
    }

    GetActualFileFullData(fileFrm: any) {
        this.file = this.files.filter(x => x.FID == fileFrm._value.FID)[0];
        let val = this.file.ActualFilePath.split('/')[this.file.ActualFilePath.split('/').length - 1];
        let fileNameOnly = val.split('.')[0];
        let localFileData = '';
        ...
        this._sampleFileService.getLoginInfo(Global.BASE_FILESAVE_ENDPOINT, fileNameOnly)
            ...
            error => this.msg = <any>error,
            () => this.ViewSampleFile(fileFrm._value.FID)
            );
